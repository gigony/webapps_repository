<?php
 ...
 if($_POST["addparent"] == 1)
 {
  if($_POST["username"] != "" && $_POST["fname"] != "" && $_POST["lname"] != "" && $_POST["student"] != "")
  {
   ...
   $query = mysql_query("INSERT INTO parents (userid, fname, lname) VALUES('$_POST[username]', '$_POST[fname]', '$_POST[lname]')")
   ...
   $query = mysql_query("INSERT INTO parent_student_match (parentid, studentid) VALUES( '$parentid', '$_POST[student]')")
 ...
 if($_POST["editparent"] == 1 && $_POST["username"] != "" && $_POST["fname"] != "" && $_POST["lname"] != "" && $_POST["student"] != "")
 {
  ...
  $rset1 = mysql_query("SELECT parentid from parent_student_match where studentid='".
	$_POST['student']."'");
  if($rset1) {
    $cnt = 0;
    while($idresult =  mysql_fetch_array($rset1)) {
      if($idresult[0] != $_POST['parentid']) {
	$query3 = mysql_query("UPDATE parent_student_match set studentid=".$_POST['student']." WHERE parentid=".$_POST['parentid']." LIMIT 1 ");
      }
      $cnt++;
    }
    if($cnt == 0) {
      $query2 = mysql_query("SELECT matchid from parent_student_match where parentid=".
		$_POST['parentid']." LIMIT 1");
      if($query2) {
	$resarr = mysql_fetch_array($query2);
	if($resarr['matchid']) {
	  $query3 = mysql_query("UPDATE parent_student_match SET studentid=".$_POST['student'].
		" WHERE parentid=".$_POST['parentid']." LIMIT 1");
	  }
        }
	else { // no prior linkage parent->child
          $query4 = mysql_query("INSERT INTO parent_student_match (parentid, studentid)".
		" VALUES (".$_POST['parentid'].", ".$_POST['student'].")");
	}
     }
   }
 }
