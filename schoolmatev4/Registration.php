<?php
 ...
 if($_POST["addreg"] == 1)
 {
  ...
  $cinfo_days = preg_split('//', $cinfo[2], -1, PREG_SPLIT_NO_EMPTY);
 ...
 if($curregs[0] == "")
 {
  ...
  if($insert)
  {
   ...
   $query = mysql_query("INSERT INTO registrations (courseid, studentid, semesterid, termid) VALUES('$_POST[class]','$_POST[student]', '$_POST[semester]', '$id[0]')")
	 or die("Registration.php: Unable to add the new registration - ".mysql_error());

   // Check to see if the class is a full year //
   $query = mysql_query("SELECT secondcourseid FROM courses WHERE coursename = $_POST[class]");
